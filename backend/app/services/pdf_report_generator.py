...
def _warning_banner(text: str, styles, accent: str, background: str) -> Table:
...
def _build_header(project_data: Dict[str, Any], styles: Dict[str, ParagraphStyle]) -> List[Any]:
    story: List[Any] = []
...
def _build_warning_banner(run_data: Dict[str, Any], styles: Dict[str, ParagraphStyle]) -> List[Any]:
    story: List[Any] = []
    if not run_data.get("is_publishable", False):
        story.append(_warning_banner(
...
def _build_project_data(project_data: Dict[str, Any], styles: Dict[str, ParagraphStyle]) -> List[Any]:
    story: List[Any] = []
    ...
    rows = [
        ...
        ("Área do Terreno", _fmt(project_data.get("lot_area"), "m²"),
         "Área Construída", _fmt(project_data.get("built_area"), "m²")),
        ("Recuo Frontal", _fmt(project_data.get("front_setback"), "m"),
         "Recuo Fundos", _fmt(project_data.get("rear_setback"), "m")),
        ("Taxa de Ocupação (derivada)", _fmt(project_data.get("occupancy_rate"), "%", 1),
         "Permeabilidade", _fmt(project_data.get("permeability_rate"), "%", 1)),
        ("Pavimentos", _fmt(project_data.get("floors")),
...
def _build_validation_results(validations: List[Dict[str, Any]], styles: Dict[str, ParagraphStyle]) -> List[Any]:
    story: List[Any] = []
...
def _build_unverifiable_items(validations: List[Dict[str, Any]], styles: Dict[str, ParagraphStyle]) -> List[Any]:
    story: List[Any] = []
...
def _build_disclaimers(styles: Dict[str, ParagraphStyle]) -> List[Any]:
    story: List[Any] = []
...
def _build_provenance(run_data: Dict[str, Any], validations: List[Dict[str, Any]], styles: Dict[str, ParagraphStyle]) -> List[Any]:
    story: List[Any] = []
...
def generate_pdf(
    ...
    story: List[Any] = []
    story.extend(_build_header(project_data, styles))
    story.extend(_build_warning_banner(run_data, styles))
    story.extend(_build_project_data(project_data, styles))
    story.extend(_build_validation_results(validations, styles))
    story.extend(_build_unverifiable_items(validations, styles))
    story.extend(_build_disclaimers(styles))
    story.extend(_build_provenance(run_data, validations, styles))
