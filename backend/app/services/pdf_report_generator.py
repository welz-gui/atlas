...
class RegulatoryReportGenerator:
    ...
    def _warning_banner(self, text: str, accent: str, background: str) -> Table:
        table = Table([[Paragraph(text, self.styles["normal"])]], colWidths=[523])
        table.setStyle(
            TableStyle(
                [
    ...
    def _build_header(self) -> List[Any]:
        story: List[Any] = []
        story.append(
            Paragraph("<b>ATLAS</b> — PRÉ-ANÁLISE URBANÍSTICA", self.styles["title"])
        )
        story.append(
            Paragraph(
        ...
        municipality = self.project_data.get("city_name", "—")
        story.append(
            Paragraph(
                f"Jurisdição de referência: {municipality} / {self.project_data.get('state', '—')} "
                f"— Zona {self.project_data.get('zone', '—')}",
                self.styles["subtitle"],
            )
        )
    ...
    def _build_warning_banner(self) -> List[Any]:
        story: List[Any] = []
        if not self.run_data.get("is_publishable", False):
            story.append(
    ...
    def _build_project_data(self) -> List[Any]:
        story: List[Any] = []
        story.append(Paragraph("1. DADOS DO EMPREENDIMENTO", self.styles["h2"]))
        municipality = self.project_data.get("city_name", "—")
        rows = [
            (
                "Nome do Projeto",
                str(self.project_data.get("name", "—")),
                "Município / Zona",
                f"{municipality} / {self.project_data.get('zone', '—')}",
            ),
            (
                "Área do Terreno",
                self._fmt(self.project_data.get("lot_area"), "m²"),
                "Área Construída",
                self._fmt(self.project_data.get("built_area"), "m²"),
            ),
            (
                "Recuo Frontal",
                self._fmt(self.project_data.get("front_setback"), "m"),
                "Recuo Fundos",
                self._fmt(self.project_data.get("rear_setback"), "m"),
            ),
            (
                "Taxa de Ocupação (derivada)",
                self._fmt(self.project_data.get("occupancy_rate"), "%", 1),
                "Permeabilidade",
                self._fmt(self.project_data.get("permeability_rate"), "%", 1),
            ),
            (
                "Pavimentos",
                self._fmt(self.project_data.get("floors")),
                "Linha de Base Oficial",
                "SIM" if self.project_data.get("is_official_baseline") else "NÃO",
            ),
            (
                "Versão do projeto",
                f"v{self.project_data['version_number']}"
                if self.project_data.get("version_number")
                else "—",
                "Situação da versão",
                str(self.project_data.get("version_state") or "—").replace("_", " "),
            ),
        ]
        proj_table = Table(
            [
                [
                    Paragraph(f"<b>{a}:</b>", self.styles["bold"]),
                    Paragraph(b, self.styles["normal"]),
                    Paragraph(f"<b>{c}:</b>", self.styles["bold"]),
                    Paragraph(d, self.styles["normal"]),
                ]
                for a, b, c, d in rows
            ],
    ...
    def _build_validation_results(self) -> List[Any]:
        story: List[Any] = []
        story.append(
            Paragraph("2. VERIFICAÇÃO DETERMINÍSTICA DE REGRAS", self.styles["h2"])
        )

        header = [
            Paragraph(f"<b>{h}</b>", self.styles["bold"])
        ...
        for index, item in enumerate(self.validations, start=1):
            status = str(item.get("status", ""))
            if status not in cached_status_paragraphs:
                label, color = STATUS_PRESENTATION.get(
                    status, (status.upper(), "#334155")
                )
                cached_status_paragraphs[status] = Paragraph(
                    f"<font color='{color}'><b>{label}</b></font>", self.styles["bold"]
                )
            ...
            table_rows.append(
                [
                    Paragraph(str(item.get("rule_title", "")), self.styles["normal"]),
                    Paragraph(
                        str(item.get("expected_value", "")), self.styles["normal"]
                    ),
                    Paragraph(str(item.get("actual_value", "")), self.styles["normal"]),
                    cached_status_paragraphs[status],
                    Paragraph(citation, self.styles["small"]),
                ]
            )
    ...
    def _build_unverifiable_items(self) -> List[Any]:
        story: List[Any] = []
        unverifiable = [
            v for v in self.validations if v.get("status") == "nao_verificavel"
        ]
        story.append(Paragraph("3. ITENS NÃO VERIFICÁVEIS", self.styles["h2"]))
        if unverifiable:
            story.append(
                self._warning_banner(
            ...
            unverifiable_rows = [
                [
                    Paragraph("<b>Item</b>", self.styles["bold"]),
                    Paragraph("<b>Motivo</b>", self.styles["bold"]),
                    Paragraph("<b>Evidência necessária</b>", self.styles["bold"]),
                ]
            ]
            for item in unverifiable:
                unverifiable_rows.append(
                    [
                        Paragraph(
                            str(item.get("rule_title", "")), self.styles["normal"]
                        ),
                        Paragraph(str(item.get("details", "")), self.styles["normal"]),
                        Paragraph(
            ...
            story.append(
                Paragraph(
                    "Nenhum item ficou sem verificação nesta análise.",
                    self.styles["normal"],
                )
            )
        story.append(Spacer(1, 14))
    ...
    def _build_disclaimers(self) -> List[Any]:
        story: List[Any] = []
        story.append(Paragraph("4. RESSALVAS E LIMITAÇÕES", self.styles["h2"]))
        for index, text in enumerate(DISCLAIMERS, start=1):
            story.append(Paragraph(f"{index}. {text}", self.styles["normal"]))
            story.append(Spacer(1, 4))
        story.append(Spacer(1, 10))
        return story

    def _build_provenance(self) -> List[Any]:
        story: List[Any] = []
        story.append(Paragraph("5. PROVENIÊNCIA DA ANÁLISE", self.styles["h2"]))
        verified = sum(1 for v in self.validations if v.get("source_is_verified"))
        emitted_at = datetime.utcnow().strftime("%d/%m/%Y %H:%M:%S UTC")
        provenance = [
            ("Identificador da análise", str(self.run_data.get("id", "—"))),
            (
                "Hash SHA-256 do conteúdo analisado",
                str(self.run_data.get("content_hash", "—")),
            ),
            (
                "Versão do catálogo regulatório",
                str(self.run_data.get("catalog_version", "—")),
            ),
            (
                "Versão do motor de regras",
                str(self.run_data.get("engine_version", "—")),
            ),
            ("Regras aplicadas", str(len(self.validations))),
            (
                "Regras com fonte legal conferida",
                f"{verified} de {len(self.validations)}",
            ),
            (
                "Validação técnica humana",
                "PENDENTE" if not self.run_data.get("is_publishable") else "CONCLUÍDA",
            ),
        ...
        provenance_table = Table(
            [
                [
                    Paragraph(f"<b>{k}</b>", self.styles["small"]),
                    Paragraph(f"<font face='Courier'>{v}</font>", self.styles["small"]),
                ]
    ...
    def generate_pdf(
        ...
        generator = cls(project_data, validations, run_data)
    ...
    def _generate(self) -> bytes:
        ...
        doc = SimpleDocTemplate(
            ...
            topMargin=36,
            bottomMargin=36,
            title=f"Pré-análise urbanística — {self.project_data.get('name', '')}",
            author="Atlas",
        )

        story: List[Any] = []
        story.extend(self._build_header())
        story.extend(self._build_warning_banner())
        story.extend(self._build_project_data())
        story.extend(self._build_validation_results())
        story.extend(self._build_unverifiable_items())
        story.extend(self._build_disclaimers())
        story.extend(self._build_provenance())
