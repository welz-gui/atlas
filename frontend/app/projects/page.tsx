      ...
      <div className="glass-panel rounded-2xl p-6 w-full max-w-3xl space-y-5 my-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <label className="block max-w-md">
            <span className="text-[11px] font-semibold text-slate-400 block mb-1.5">Município do empreendimento</span>
            <select value={municipalityCode} onChange={(event) => setMunicipalityCode(event.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-800 text-sm text-white focus:border-cyan-500 outline-none">
              {MUNICIPALITIES.map((municipality) => (
                <option key={municipality.city_ibge} value={municipality.city_ibge}>{municipality.city_name}/{municipality.state}</option>
              ))}
            </select>
            <span className="text-[10px] text-slate-500 mt-1 block">Define quais normas municipais serão aplicadas ao projeto.</span>
          </label>
